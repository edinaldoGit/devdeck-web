...
function handleSave() { emit('save', form.value) }
function handleDelete() { if (confirm('Tem certeza?')) emit('delete', form.value.id) }

function triggerFileInput() {
  if ((form.value.images?.length || 0) >= 5) return alert('Máximo de 5 fotos.')
...
async function onFileSelected(event: Event) {
  ...
  if (target.files && target.files[0]) {
    ...
    } catch (error) { console.error(error); alert('Erro no upload') } finally { isUploading.value = false; if (fileInput.value) fileInput.value.value = '' }
  }
}

function removeImage(index: number) { form.value.images?.splice(index, 1) }
...
function removeTag(index: number) { form.value.tags?.splice(index, 1) }
</script>
          ...
          <div class="modal-body">
              ...
              <div class="gallery-grid">
                ...
                >
                  ...
                  <div class="item-overlay">
                    ...
                    <div class="actions">
                      <button v-if="index !== 0" @click.stop="makeCover(index)" class="btn-star" title="Definir como Capa">
                        <i class="bi bi-star-fill"></i>
                      </button>
                      <button class="btn-trash" @click.stop="removeImage(index)" title="Remover">
            ...
            <div class="form-grid">
              ...
              <div class="form-group">
                ...
                <input v-model="form.title" type="text" class="input-premium" placeholder="Nome do projeto" />
              </div>

              <div class="form-group">
                <label>Descrição Curta</label>
                <input v-model="form.shortDescription" type="text" class="input-premium" placeholder="Breve resumo" />
              </div>

              <div class="form-group full-width">
                <label>Descrição Detalhada</label>
                <textarea v-model="form.detailedDescription" rows="5" class="input-premium area" placeholder="Detalhes completos..."></textarea>
              </div>

              <div class="form-group">
                <label><i class="bi bi-github"></i> Repositório</label>
                <input v-model="form.linkRepo" type="text" class="input-premium" placeholder="URL do GitHub" />
              </div>

              <div class="form-group">
                <label><i class="bi bi-globe"></i> Deploy (Demo)</label>
                <input v-model="form.linkDeploy" type="text" class="input-premium" placeholder="URL do site no ar" />
              </div>
...
.modal-container { 
  ...
  animation: modalEnter 0.3s cubic-bezier(0.2, 0.8, 0.2, 1);
  overflow: hidden;
}
...
.btn-close { 
  background: #f8fafc; border: none; width: 40px; height: 40px; border-radius: 50%; 
  color: #64748b; cursor: pointer; transition: 0.2s; display: flex; align-items: center; justify-content: center; font-size: 1.2rem;
}
...
.upload-box {
  height: 120px; border: 2px dashed #cbd5e1; border-radius: 16px;
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  cursor: pointer; color: #94a3b8; transition: all 0.2s; background: #f8fafc;
}
...
.item-overlay { 
  position: absolute; inset: 0; background: rgba(0,0,0,0.5); opacity: 0; transition: 0.2s; 
...
.form-group label { 
  display: block; margin-bottom: 0.6rem; font-weight: 700; font-size: 1rem; color: #334155; 
}
...
.btn-delete { 
  ...
  padding: 0.75rem 1.5rem; border-radius: 12px; transition: 0.2s;
}
.btn-delete:hover { background: #fef2f2; border-color: #fee2e2; }

.btn-cancel { 
  padding: 0.8rem 1.8rem; border: 1px solid #cbd5e1; background: white; border-radius: 12px; 
  cursor: pointer; font-weight: 600; color: #475569; font-size: 1rem; transition: 0.2s; 
}
.btn-cancel:hover { background: #f1f5f9; color: #1e293b; border-color: #94a3b8; }

.btn-save { 
  padding: 0.8rem 2.2rem; border: none; border-radius: 12px; cursor: pointer; font-weight: 700; color: white; font-size: 1rem;
